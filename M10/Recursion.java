...
public class Recursion
{
	...
	public static List<Integer> collatzList(int n)
	{
		...
		else
		{
			...
			//Adds all lists including next collatz number
			list.addAll(collatzList(n));
			
//			Iterative version
//			while(n != 1)
//			{
//				list.add(nextCollatzNumber(n));
//				
//				n = nextCollatzNumber(n);
//			}
			...
			final double interestRate, final int periods)
	{
		...
		else
		{
			...
			double startBalanceForNthPeriod = calculateEndBalance(startBalance + startBalance * interestRate , interestRate, periods - 1);
			...
			final int maxPopulation, final double growthRate, final int periods)
	{
		...
		{
			//Calculates growth rate for given amount of periods
			double growthRateForNthPeriod = growthRate * (1 - (double)startPopulation/maxPopulation);
			
			//Calculates the population with the growth rate with given period
			int popGrowthCurrent = (int)(startPopulation + growthRateForNthPeriod * startPopulation);
			
			//Returns the end population
			return calculateEndPopulation(popGrowthCurrent, maxPopulation, growthRate, periods - 1);
		}
	}
